...
def _halt_if_requested(job_id: str, stats: dict[str, Any]) -> bool:
    """Cooperatively stop legacy pipeline at safe stage boundaries."""
    from app.services.job_control import check_job_cancelled, check_job_paused

    if check_job_cancelled(job_id):
        log.info("Pipeline cancelled at safe boundary: %s", job_id)
        return True
    if check_job_paused(job_id):
        _update_job_status(job_id, status="paused", stats=stats)
        log.info("Pipeline paused at safe boundary: %s", job_id)
        return True
    return False
...
def run_pipeline(self, job_id: str, upload_paths: list[str] | None = None) -> dict[str, Any]:
    ...
    if _halt_if_requested(job_id, stats):
        return {"ok": True, "paused": True, "stats": stats}
