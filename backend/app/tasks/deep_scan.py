...
def run_deep_scan(self, job_id: str) -> dict[str, Any]:
    ...
    for batch_idx in range(0, len(profile_urls), _BATCH_SIZE):
        ...
        if check_job_cancelled(job_id) or check_job_paused(job_id):
            _update_job(job_id, status="paused" if check_job_paused(job_id) else "phase1_done", stats=stats)
            return {"ok": True, "paused": check_job_paused(job_id), "stats": stats}
